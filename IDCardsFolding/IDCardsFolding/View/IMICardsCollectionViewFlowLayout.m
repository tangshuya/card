//
//  IMICardsCollectionViewFlowLayout.m
//  WifireVportProject
//
...
#define kCellHeight 217 /// cell 高度
#define kCellSpace 57 // cell0Top to cell1Top
#define kComeUpAnimationDuration 0.25
#define kBottomAutoScrollDistance 217
#define kTopAutoScrollDistance 57
#define kCanMoveDistance 30
#define kExpandBottomHeight ([UIScreen mainScreen].bounds.size.height-282） // 65+217展开cell的bottom距离屏幕bottom的距离,称之为footer吧
#define kExpandBottomFirstCellMarginOfTop 15 // footer里面第一个cell距离顶部的距离
...
@interface IMICardsCollectionViewFlowLayout()
...
@property(weak, nonatomic) IMICardsCollectionViewCell *currentCell;
...
@property(assign, nonatomic) CGFloat bottomHeight; // bottomHeight
@end

@implementation IMICardsCollectionViewFlowLayout
...
- (instancetype)init
{
    ...
    if (self) {
//        65+217已经占据的高度
        CGFloat bottomHeight = ([UIScreen mainScreen].bounds.size.height-282);
        ...
        if (row * kCellSpace >= offsetY - kCellSpace) {
            ...
            if (self.isExpand) {
                if (indexPath.item == self.currentIndexPath.item) {
                    cellRect = CGRectMake(0, offsetY, kCollectionViewWidth - inset.right, kCollectionViewHeight - self.bottomHeight);
                    self.currentCellShrinkMarginLeft = left;
                } else {
                    cellRect = CGRectMake(left, offsetY + (kCollectionViewHeight - self.bottomHeight) + top, kCollectionViewWidth - inset.right - left * 2, kCellHeight);
                    shrinkCellIndex++;
...
- (void)collectionViewCell:(IMICardsCollectionViewCell *)cell handlerPanGesture:(UIPanGestureRecognizer *)ges {
    ...
    switch (ges.state) {
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            CGRect frame = cell.frame;
            CGFloat pecent = (frame.origin.y - offsetY) / (kCollectionViewHeight - self.bottomHeight);
            ...
            frame = CGRectMake(left, frame.origin.y + translate.y, kCollectionViewWidth - inset.right - left * 2,kCollectionViewHeight - self.bottomHeight );
            cell.frame = frame;
        ...
        case UIGestureRecognizerStateFailed:
        {
            ...
            if (cell.frame.origin.y > kCollectionViewHeight / 2 - self.bottomHeight / 2) {
                // 结束
                [self closeCell];
            } else {
                // 复位
                [UIView animateWithDuration:kComeUpAnimationDuration animations:^{
                    cell.frame = CGRectMake(0, offsetY, kCollectionViewWidth - inset.right, kCollectionViewHeight - self.bottomHeight);
                }];
            }
        }
            break;
...
- (void)handlerMoveItemAction {
    if (!self.isMoveing) {
        ...
        IMICardsCollectionViewCell *preferCell;
        preferIndexPath = [NSIndexPath indexPathForItem:self.currentIndexPath.item + 1 inSection:self.currentIndexPath.section];
        preferCell = (IMICardsCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:preferIndexPath];
...
- (void)expandCellWithIndexPath:(NSIndexPath *)indexPath {
    self.currentIndexPath = indexPath;
    self.currentCell = (IMICardsCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
...
- (void)closeCell {
    ...
    UIEdgeInsets inset = self.collectionView.contentInset;
    [UIView animateWithDuration:kComeUpAnimationDuration animations:^{
        self.currentCell.frame = CGRectMake(self.currentCellShrinkMarginLeft, kCollectionViewHeight, kCollectionViewWidth - inset.right - self.currentCellShrinkMarginLeft * 2, kCollectionViewHeight - self.bottomHeight);
    }completion:^(BOOL finished) {
        [self closeCellWithoutAnimation];
    }];
}
